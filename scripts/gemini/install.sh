...
# Fallback installer for Gemini CLI when Rust toolchain is unavailable.
# Installs skills via gemini CLI (preferred) or direct copy.
# Agents and rules are not applicable — Gemini uses GEMINI.md for instructions.
#
...
print_usage() {
    ...
    echo ""
    echo "Fallback installer for Gemini CLI — copies skills without the Rust toolchain."
    echo ""
    echo "Options:"
...
while [ $# -gt 0 ]; do
    case "$1" in
        --version)      echo "install.sh ${VERSION} (gemini)"; exit 0 ;;
...
case "$SCOPE" in
    workspace)  SKILLS_DST=".gemini/skills" ;;
    user)       SKILLS_DST="${HOME}/.gemini/skills" ;;
    *)          echo "Error: invalid scope '${SCOPE}' (use workspace or user)" >&2; exit 1 ;;
esac

# --- Skills ---
# Gemini skills use the same directory structure as Claude (SKILL.md + companions).
# Names are kebab-cased in Gemini conventions.
# The gemini CLI can install skills natively, but for fallback we copy directly.

to_kebab_case() {
    echo "$1" | sed -E 's/([a-z0-9])([A-Z])/\1-\2/g' | tr '[:upper:]' '[:lower:]' | tr ' _' '-' | sed 's/--*/-/g'
}
...
install_skills_from_dir() {
    ...
    for skill_dir in "$skills_root"/*/; do
        ...
        local target="${SKILLS_DST}/${kebab_name}"
...
echo ""
echo "Gemini fallback install complete (scope: ${SCOPE})."
echo "  Skills: ${total}"
echo ""
echo "Note: Gemini CLI uses GEMINI.md for project instructions (not rules/ directory)."
echo "Agents use gemini agent format — run 'gemini init' for GEMINI.md generation."
